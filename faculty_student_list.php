...
// Get the current faculty ID from the session (assuming it's stored there)
$faculty_id = $_SESSION['faculty_id'];

// Fetch the sections the faculty is assigned to
$sections_query = "
    SELECT sec.section_id, sec.section_name 
    FROM sections sec
    INNER JOIN section_subject ss ON sec.section_id = ss.section_id
    INNER JOIN faculty_section_subject fss ON ss.section_subject_id = fss.section_subject_id
    WHERE fss.faculty_id = '$faculty_id'
";
$faculty_sections = $conn->query($sections_query);
$section_ids = [];
$sections = [];

while ($row = $faculty_sections->fetch_assoc()) {
    $section_ids[] = $row['section_id'];
    $sections[] = $row; // Save sections for filter dropdown
}

if (empty($section_ids)) {
    echo "No sections assigned to the faculty.";
    exit();
}
...
$query = "
    ...
    WHERE sec.section_id IN (" . implode(',', $section_ids) . ")
";
                    ...
                    <tbody>
                        <?php while ($student = $result->fetch_assoc()) : ?>
                            <tr>
                                ...
                                <td>
                                    <a href="faculty_student_profile.php?id=<?php echo $student['student_id']; ?>" class="btn view-btn">View Profile</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
